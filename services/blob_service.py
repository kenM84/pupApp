...
async def configure():
    ...
    try:
        account_name = await AuthService.get_secret("Storage--AccountName")
        key = await AuthService.get_secret("Storage--AccessKey")
        connect_str = (
        ...
        genaisys_blob_service_client = BlobServiceClient.from_connection_string(connect_str)
...
def download_genaisys_blob_by_http(url: str) -> str:
    ...
    try:
        ...
        logger.error("HTTP blob download failed for URL %s: %s", url, e, exc_info=True)
        raise


async def download_genaisys_blob(container: str, path: str) -> bytes:
    ...
    container_client = genaisys_blob_service_client.get_container_client(container)
...
async def upload_genaisys_blob(container: str, path: str, data) -> bool:
    """Upload blob without overwriting existing blobs."""
    global genaisys_blob_service_client
    if genaisys_blob_service_client is None:
        await configure()

    logger.debug("Uploading blob '%s' to container '%s' (no overwrite).", path, container)
    container_client = genaisys_blob_service_client.get_container_client(container)
    blob_client: BlobClient = container_client.get_blob_client(path)
...
async def upload_or_replace_genaisys_blob(container: str, path: str, data) -> bool:
    """Upload blob and overwrite if it exists."""
    global genaisys_blob_service_client
    if genaisys_blob_service_client is None:
        await configure()

    logger.debug("Uploading blob '%s' to container '%s' with overwrite.", path, container)
    container_client = genaisys_blob_service_client.get_container_client(container)
    blob_client: BlobClient = container_client.get_blob_client(path)
    blob_client.upload_blob(data, overwrite=True)
    logger.info("Uploaded (with overwrite) blob '%s' to container '%s'.", path, container)
    return True
